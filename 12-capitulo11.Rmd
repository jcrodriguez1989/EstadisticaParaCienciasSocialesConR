...
desarrolladas inicialmente por Sir Ronald Fisher (@Fisher) a las que llamó
...
Para poner a prueba la hipótesis tomaremos una muestra de niños y niñas que provienen de hogares de alto nivel educativo y veremos si su rendimiento es superior a 60 puntos. Supongamos que en la muestra hallamos una media de 62 puntos,
...
poblacional de quienes cursan con el nuevo plan, que no están todos accesibles, porque hay quienes están cursando y quienes lo harán en el futuro (el nuevo plan sigue vigente), por lo que de esa población solo puede conocerse a una muestra de quienes ya han egresado y ver cuánto tiempo han tardado en terminar la carrera.
...
ggplot(data = data.frame(z = c(6.7, 7.9)), aes(z)) +
  ...
  geom_text(aes(label = TeX("$\\bar{x}$", output = "character")), x = 8, y = -.1, parse = TRUE) +
  annotate(geom = "rect", xmin = 7.298, xmax = 7.302, ymin = -.05, ymax = .05, col = "red") +
  geom_text(aes(label = TeX("$\\bar{x}_{obs} = 7.5$", output = "character")), x = 7.7, y = 1.3, parse = TRUE) +
...
generacional, cómo vivían los padres y madres en relación a ellos y cómo vivirán
los hijos e hijas con respecto a ellos (expectativa futura). En el año 2004, el
58.2% de los argentinos encuestados consideraba que los hijos e hijas vivirían
...
que componían la muestra consideraban que sus hijos e hijas vivirían mejor que ellos, con lo que, la proporción en la muestra es:
...
interés por la política. En el estudio del año 2004, el 11% de las personas encuestadas en Argentina manifestó estar muy interesado en la política. Si bien no ha habido notables cambios en la última década con respecto al interés de la ciudadanía en la política, una serie de indicadores llevaron a los investigadores a considerar que el interés en esta cuestión podría haber disminuido. Las hipótesis planteadas son:

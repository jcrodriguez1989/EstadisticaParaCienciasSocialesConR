...
Esta información es suficiente para aplicar el modelo binomial, que se va a escribir formalmente como $$B(n, p, x)$$
...
Si $x$ tiene distribución normal con media 5 y desviación estándar 2, la $P(x<6.3)$, redondeada a cuatro decimales es: 
```{r echo=TRUE}
round(pnorm(6.3,5,2),4)
```

Y para un intervalo de valores, como $P(6<x<8)$:
```{r echo=TRUE}
...
Para un área superior, $P(t_{25}>1.8)$  
```{r  echo=TRUE}

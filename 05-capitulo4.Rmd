...
ilustra el primer caso es la relación entre el comportamiento infantil y el trato que recibe de sus progenitores. Puede interpretarse que, quienes tienen comportamientos revoltosos, responden así a la escasa atención que se les brinda; o bien leer la forma en que las madres y los padres los tratan como consecuencia de la mala conducta que manifiestan. El orden en que se plantee la relación está influido por la posición teórica del investigador.  
...
La frecuencia relativa $0.380$ es el riesgo de tener cardiopatía isquémica si se es hipertenso, mientras que $0.096$ es el mismo riesgo para personas que no padecen hipertensión. Dicho como porcentaje, el $38\%$ de los hipertensos tuvo CI, mientras que solo el $9.6\%$ de los no hipertensos tuvo CI. La comparación de ellos es el riesgo relativo:
...
levels(one$P16)<-c(NA, "si", "no", NA)
one$P16<-factor(one$P16)
...
```{r}
kableExtra::kable_styling(
  knitr::kable(lectura_rendim_absolutas, format = "markdown", booktabs = TRUE, align = "lccc", caption = "Distribución conjunta de la lectura extraescolar y el resultado en la prueba de lengua"),
  latex_options = "striped"
) %>%
...
```{r}
kableExtra::kable_styling(
  knitr::kable(lectura_rendim_relativas, format = "markdown", booktabs = TRUE, align = "lccc", caption = "Distribución conjunta de la lectura extraescolar y el resultado en la prueba de lengua"),
  latex_options = "striped"
) %>%
  footnote(general = "Fuente: ONE 2000")
...
<!-- si la opción B sale bien corregir estos números-->
...
Estos valores son los que se usan para calcular el riesgo relativo, el cociente entre 26.4 y 18.8 es 1.4, por lo que diremos que hay 1.4 veces más chances de obtener bajos resultados en Lengua entre quienes no leen que entre los que sí lo hacen.
...
```{r}
sexo_por_estado<-table(eph.3.18.validos.estado$sexo,
                       eph.3.18.validos.estado$ESTADO)
```

...
- El separador de campos: suelen ser comas, punto y comas o pipe | (alt124 en windows). Si los campos están separados por comas. se indica sep="," y así con los demás separadores, siempre entrecomillados.
...
Para leer la base de prueba, que se llama base2019.txt, que tiene los campos separados por punto y comas, primero se ubica la ruta del archivo:
...
head(base.ejemplo)
```
Por defecto muestra las seis primera filas, en todas  las columnas (se usaron dos bloques porque no cabe horizontalmente).  
...
Si pedimos nuevamente el encabezado, tenemos:
```{r echo=TRUE}
head(base.ejemplo)
```
Ahora hicieron falta cuantro bloques (cada uno numera del 1 al 6, las seis primeras filas), porque con los nuevos nombres la matriz quedó más ancha.  

### Las variables

Cada variable es una columna de la matriz, y para acceder a cada una y operar con ellas, se usa el signo "pesos" luego del nombre de la matriz. Por ejemplo la variable "sexo" La codificación numérica de la variable sexo (1 y 2 para varones y mujeres respectivamente), fue útil para la recolección y la carga de los datos, pero ahora necesitamos tratarla commo  lo que es. En efecto:
```{r echo=TRUE}
...
Elegimos la primera opción, es decir que operamos sobre la misma variable sexo. Primero la volvemos factor:

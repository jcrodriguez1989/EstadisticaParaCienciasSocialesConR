...
efecto_chico_alpha5_s1<-ggplot(data.frame(x = c(-3.5,7.5)), aes(x)) +
  ...
  stat_function(fun = dnorm, args = list(mean = 0, sd = s), fill="grey17",
                xlim = c(qnorm(1-alpha),4),
                geom = "area")+
  stat_function(fun = dnorm, args = list(mean = mu_1, sd = s), col="grey57")+
  stat_function(fun = dnorm, args = list(mean = mu_1, sd = s), 
                xlim = c(-4,qnorm(1-alpha)),fill="grey57", alpha=.5, geom = "area")+geom_vline(xintercept = 0, col="grey17")+
...
Luego se indica el tipo de prueba; `pwr.t.test` para una prueba t, y los valores de: significación, potencia, tamaño del efecto y lateralidad de la prueba.  
```{r}
pwr.t.test( sig.level = .05, power= .8, d=0.85, alternative = "greater")

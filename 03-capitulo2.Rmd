...
## Hacerlo en R  
Volvemos  sobre la base que se trabajó en el capítulo anterior:  
...
Las tablas de distribución de frecuencias se solicitan con un comando del paquete {base} que se llama `table`, que tiene como argumento la variable cuya tabla se pide. Así, para la tabla de sexo:
```{r echo=TRUE}
...
Las opciones de `useNA` son "$no$", que se la que se aplica por defecto, "$ifany$" (si hay alguno) y la que hemos usado en este ejemplo, "$always$" (siempre).  
...
### Recategorización  
La construcción de categorías a partir de una variable continua, se realiza con el comando `cut`, cuyo argumento contiene a la variable que se va a recategorizar, y una instrucción `breaks=` que especifica el modo de hacer los cortes, esta instrucción difiere según los tipos de racategorización.  
...
Ahora la base tiene 4991 observaciones.

#### Intervalos de amplitud igual  

Para que lo ontervalos sean iguales, solo se indica la cantidad de ellos que se quiere, en este caso, cuatro:  
```{r echo=TRUE}
...
Allí se han indicado todos los cortes: el mínimo valor de la variable, los cuantiles y el máximo.  
La tabla es:
...
```{r echo=TRUE}
base.ejemplo$edad4t<-cut(base.ejemplo$edad, breaks = c(min(base.ejemplo$edad),18, 23, 35, max(base.ejemplo$edad)))
...
El paquete {base} contiene rutinas elementales de graficación, sin embargo, desde aquí vamos a empezar a usar un paquete específico paraeso. TRAER LA INTRO A GGPLOT, LAS CAPAS...
```{r echo=TRUE}
library(ggplot2)
```

Para hacer un gráfico de barras, empezamos indicando sobre qué base trabajaremos, en el argumento de ggplot y agregamos una capa de barras (geom_bar) con la variable:  
...
Vamos a usar este ejemplo para ilustrar la diferencia entre "fijar" y "mapear" a un atributo estético, que en este caso será el color de relleno de las barras. Fijar el color es simplemente establecerlo como preferencia para todas las barras, es un argumento de la capa geom_bar:
...
Por el contrario, "mapear" es pedir que el color de relleno cambie en base a las categorías de la variable, éste es un argumento de aes:  
```{r echo=TRUE}
ggplot(subset(base.ejemplo, is.na(base.ejemplo$red.usada)==FALSE))+
...
El mapeo es muy útil para introducir otra variable y ver las diferencias. Por ejemplo, el uso de redes según sexo:  
```{r echo=TRUE}
ggplot(subset(base.ejemplo, is.na(base.ejemplo$red.usada)==FALSE & is.na(base.ejemplo$sexo)==FALSE))+
  geom_bar(aes(red.usada, fill=sexo))+coord_flip()
```

El histograma tiene su propia capa, para la variable edad:  
...
Por defecto, se toman 30 intervalos, lo cual puede ajustarse, por ejemplo, a 15:  
```{r echo=TRUE}
ggplot(base.ejemplo)+
...
Hay muchas opciones para mejorar la visualización de estos gráficos, que s eirán presentando más adelante.
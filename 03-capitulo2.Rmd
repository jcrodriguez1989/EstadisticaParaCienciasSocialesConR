...
Se utilizan tres criterios para recategorizar variables: intervalos de amplitud igual, proporcional y teórica.
...
Los intervalos son de la misma amplitud, 9.2 años, pero el primero de ellos tiene más del 70% de las observaciones, con lo que se pierde detalle de la distribuciión.

#### Intervalos de amplitud proporcional

Este criterio busca que los intervalos incluyan aproximadamente a la misma cantidad de casos, por lo que su amplitud puede ser diferente. En el capítulo siguiente se verá que los puntos para establecer los cortes de intervalos, se llaman cuantiles. Por ahora interesa que con este criterio se logran grupos homogéneos en términos de cantidad de observaciones. La edad de los estudiantes, con cuatro intervalos resulta así categorizada:
...
Si la variable es continua, se trata de la **ojiva de Galton**, las frecuencias se van acumulando gradualmente a medida que aumenta el valor de la variable. El siguiente gráfico muestra las frecuencias acumuladas de los pesos de un conjunto grande de niños (de la base **bayley**):
...
La ojiva de Galton tiene otra virtud además de la claridad visual, ya que permite interpolar valores no observados, o que no aparecen en la tabla. Así, con el gráfico podemos responder a la pregunta ¿Qué proporción de niños tiene `r round(quantile(bayley$peso.nac, .17), 0)` g o menos? La respuesta consiste en buscar el valor `r round(quantile(bayley$peso.nac, .17), 0)` gramos en el eje x, e identificar la frecuencia acumulada que le corresponde. 
...
La salida se puede simplificar si no hay interés en un análisis puntual sobre los casos perdidos, eliminando su aparición. En el comando `freq`, el argumento `report.nas=FALSE` establece que se elimine la refencia a los casos perdidos, ya que aun cuando no haya, la tabla los menciona, con frecuencia cero.  
```{r echo=TRUE}
...
```{r echo=TRUE}
base.ejemplo=subset(base.ejemplo,
...
base.ejemplo$edad4p<-cut(base.ejemplo$edad,
                         breaks = c(min(base.ejemplo$edad),
              quantile(base.ejemplo$edad, .25),
              quantile(base.ejemplo$edad, .50),
              quantile(base.ejemplo$edad, .75),
              max(base.ejemplo$edad)))
```
...
Cada figura geométrica (`geom`) tiene asociada una transformación estadística por defecto, por lo que no es necesario especificarla, salvo que se quiera modificar. Por ejemplo si un gráfico proviene de la base de datos, la tranformación consiste en contar las ocurrencias de cada valor, ese es el `stat` por defecto. Pero si se quiere graficar desde una tabla de distribución de frecuencias, entonces en lugar de contar las veces que aparece cada categor+ia, hay que considerar las frecuencias. En este último caso, será necesario especificar una tranformación diferente.    
...
```{r echo=TRUE}
ggplot(base.ejemplo)+geom_bar(aes(red.usada))
```
...
Vamos a usar este ejemplo para ilustrar la diferencia entre "fijar" y "mapear" a un atributo estético, que en este caso será el color de relleno de las barras, con el comnado `fill` (rellenar). Fijar el color es simplemente establecerlo como preferencia para todas las barras, es un argumento de la capa `geom_bar`:
...
ggplot(subset(
  base.ejemplo, is.na(
    base.ejemplo$red.usada)==FALSE & is.na(
      base.ejemplo$sexo)==FALSE))+

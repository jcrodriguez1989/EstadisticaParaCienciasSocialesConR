...
set.seed(1)
muestras=list()
for (i in 1:200) {muestras[[i]]<-
  sample(tres_parciales$Tercero, 80)
}

muestras.df<-data.frame()
for (i in 1:200) {for (j in 1:80) {muestras.df[i,j] <-muestras[[i]][j]}}
muestras.df<-as.matrix(muestras.df)
muestras.df<-t(muestras.df)
muestras.df<-as.data.frame(muestras.df)

media=vector("numeric",200)
for (i in 1:200) {media[[i]]<-round(mean(muestras.df[,i], na.rm = TRUE),2)
}

varianza=vector("numeric",200)
for (i in 1:200) {varianza[[i]]<-round(sd(muestras.df[,i],na.rm = TRUE)^2,2)
}

muestra<-vector("numeric",200)
for (i in 1:200) {muestra[i]<-i

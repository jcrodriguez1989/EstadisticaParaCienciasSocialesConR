...
probabilístico. En consecuencia, las medidas que se calculen a partir de esos datos dependerán del azar[^65]. El concepto que desarrollamos en este capítulo es el de *variabilidad muestral*. La idea central es que cuando se extraen muestras aleatorias de una población, los resultados que se obtienen en esas muestras, son aleatorios. Dos ejemplos de ello:  
- El conjunto completo de estudiantes de Psicología en 2014 tenía una edad promedio de 26.8 años (UNC, 2014). Si ese año se hubiese extraído una muestra de 20 alumnos al azar, la edad promedio podría haber sido un número muy diferente de 26.8 años. Una posible muestra es la que tiene a  20 estudiantes muy jóvenes, por lo que la edad promedio de esa muestra va a ser menos de 26.8. Otra muestra podría tener estudiantes de más edad, por lo que promedio de esa muestra sería superior al de la población completa.  
- En las elecciones de 2019 en la provincia de Córdoba, el candidato a gobernador que ganó, obtuvo 57% de los votos. Si se selecciona aleatoriamente una muestra de 50 ciudadanos que hayan votado ese año, el porcentaje de quienes eligieron a ese cantidato pordría ser diferente a 57%. Alguna muestra podría dar cero, es decir no contener a ningún votante de ese candidato.  
...
De una población se puede obtener un número muy grande de muestras,
...
de los estimadores, hace que sean variables aleatorias.
...
La del Gráfico \@ref(fig:distrX) es la distribución en la población, en este caso es uniforme, pero podría tener cualquier otra forma. Por el contrario, la del Gráfico \@ref(fig:distrXBarra) es la distribución de las medias muestrales; es simétrica
...
hubiesen sido de tamaño tres, la varianza habría quedado reducida a la tercera
...
Resumimos los tres resultados mostrados hasta este punto:

- $E(\overline{x})=\mu$, que es lo mismo que $\mu_{\overline{x}}=\mu$
- $V(\overline{x})=\frac{\sigma^2}{n}$, que puede escribirse también $\sigma^2_\overline{x}=\frac{\sigma^2}{n}$ 
...
sueño es de $6.5 hs$ ($\ \mu = 6.5$) con varianza de $9 hs^2$ ($\ \sigma^{2} = 9$). Si eligiéramos al azar muestras de 200 personas ($n=200$) y registráramos el número promedio de horas de sueño en cada muestra, encontraríamos que la distribución de esas horas promedio sería como la siguiente:
...
Lo que cambia ahora es que nuestra variable es $\overline{x}$, su media
...
```{r}
round(1 - pnorm(2.38),4)
```

La lectura de este resultado es que menos del 0.9% de las muestras de tamaño 200 darán media muestral mayor a 7 horas.
...
ggplot(NULL, aes(c(4.5, 8.5))) +
  ...
  ylab("") + xlab("horas promedio de sueño en muestras de 30 personas") +
  scale_y_continuous(breaks = NULL) + theme_tufte()
```
...
Es decir que el 18% de las muestras de tamaño 30 darán media que supere a 7 hs. Esto ilustra que cuando las muestras son de mayor tamaño es menor la probabilidad de encontrar valores en los extremos de la distribución.
...
for (i in 1:200) {
  muestras[[i]] <-
    sample(tres_parciales$Tercero, 80)
}

muestras.df <- data.frame()
for (i in 1:200) {
  for (j in 1:80) {
    muestras.df[i, j] <- muestras[[i]][j]
  }
}
muestras.df <- as.matrix(muestras.df)
muestras.df <- t(muestras.df)
muestras.df <- as.data.frame(muestras.df)

media <- vector("numeric", 200)
for (i in 1:200) {
  media[[i]] <- round(mean(muestras.df[, i], na.rm = TRUE), 2)
}

varianza <- vector("numeric", 200)
for (i in 1:200) {
  varianza[[i]] <- round(sd(muestras.df[, i], na.rm = TRUE)^2, 2)
}

muestra <- vector("numeric", 200)
for (i in 1:200) {
  muestra[i] <- i
}
resumen_muestras <- data.frame(muestra, media, varianza)
print(head(resumen_muestras), row.names = FALSE)
```
...
```{r}
summary(resumen_muestras$media)
```

Y para las varianzas:
```{r}
summary(resumen_muestras$varianza)
```


<!-- JCR: Asi estara bien? -->
<!-- eb: ups, no vi que lo habías hecho, lo hice de nuevo-->
...
En este resumne, $n=200$ es la cantidad de muestras que se
...
de notas del tercer parcial de muestras de 80 alumnos) varía entre 5.03
y 6.54, es decir, es notablemente más restringido, tiene menos
variabilidad.  

El valor 5.79 es la media de las medias muestrales y como vemos
es muy cercana a la media poblacional, como lo es la media de las
varianzas muestrales comparada con la varianza poblacional. estos números no son idénticos, porque no se extrajeron todas las muestras, sino solo 200, pero la aproximación en adecuada para mostrar cóm se relacionan parámetros con estimadores. Eso quiere decir que la media y la varianza muestrales, son estimadores insesgados de la media y la varianza poblacionales.

Además, puede calcularse la variaza de las medias muestrales, que da:
```{r}
round(sd(resumen_muestras$media)^2,4)
```
...
```{r warning=FALSE}
ggplot(resumen_muestras) +
  geom_histogram(aes(media), fill = "green", bins = 30, col = "black") +
  ylab("frecuencia relativa") + xlim(c(1, 10)) +
  theme_tufte()
```
...
Eso quiere deci extrajéramos todas las muestras posibles de una población y

...
Si se trata de una variable de mayor nivel de medición, no hay ninguna diferencia. La variable *concepto que los docentes asignan a sus estudiantes*, que es de nivel ordinal, tiene la distribución de frecuencias siguiente:  
...
La mediana de estos datos es 7, porque es el valor que deja cuatro casos por debajo y también cuatro casos por encima. La serie simple tiene que estar ordenada para poder identificar a la mediana.
...
Cuando hay valores repetidos en la parte central es necesario hacer una corrección en la interpretación, porque ningún valor deja exactamente la mitad por encima y por debajo. Por ejemplo si la serie fuera:

$$2,\: 3,\: 7,\: 7,\: 7,\: 10,\: 15,\: 19$$

El valor 7 es la mediana, porque es *superado o igualado* por seis
valores (7, 7, 7, 10, 15 y 19), que es más de la mitad; y *supera o iguala* a cinco (2,3,7,7,7), que es la mitad.
...
la serie simple ordenada está entre quienes fueron diagnosticados a
...
comparaciones. Por ejemplo, si entre quienes cursan por la mañana la media es de 3.10 materias aprobadas y entre los que lo hacen por la tarde es de 3.90; puede decirse que el segundo grupo tiene aprobadas, en promedio, más materias que el primero aunque ninguno de sus integrantes haya aprobado 3.10 ni 3.90 materias.
...
Se observa que, aunque en todos los grupos existen caso de estudiantes con edades altas, quienes no tienen actividad laboral (inactives) se concentran en las edades más bajas.
...
total de observaciones entre las distintas categorías. Por ejemplo, si cien participantes de un experimento son clasificados según cómo sea su rendimiento en una prueba dada, en: muy bueno, bueno, regular, insatisfactorio; la distribución tendrá más
...
Diremos que aquí tenemos menos incertidumbre. En el caso extremo que todos estuvieran en la categoría "bueno", ya no hay incertidumbre, cualquier persona elegida al azar tendrá ese rendimiento; la incertidumbre es cero, pero ya no es una variable, porque solo tiene una categoría con casos, es una constante y allí la dispersión siempre es cero.  
...
($s = 4$ errores). Si un estudiante comete 6 errores ($x = 6$ errores),
...
SRIC <- (quantile(asalariades$PP08D1, .75) - quantile(asalariades$PP08D1, .25)) / 2
names(SRIC) <- "SRIC"
round(SRIC, 2)

var <- var(asalariades$PP08D1) # el cálculo de la varianza está en el paquete {base}
names(var) <- "s2"
round(var, 2)
...
CV <- sd(asalariades$PP08D1) / mean(asalariades$PP08D1) * 100 # al CV hay que definirlo
names(CV) <- "CV"
round(CV, 2)
```
...
La medida relativa de la variabilidad (el $CV$) es adecuada para comparar variables que tienen diferentes unidades, por ejemplo, sobre la base que contiene los puntajes que la muestra de niñas y niños obtuvo en  una prueba de desarrollo de Bayley cabe preguntar ¿qué es más variable, el puntaje en la subescala motora o en la mental? Es decir, ¿en cuál de esas dos medidas se diferencian más los niños y las niñas evaluadas? Leemos la base
...
```{r warning= FALSE, echo=TRUE}
cv_motora <- round(100 * sd(bayley$baymotor) / mean(bayley$baymotor), 1)
cv_mental <- round(100 * sd(bayley$bayment) / mean(bayley$bayment), 1)
```

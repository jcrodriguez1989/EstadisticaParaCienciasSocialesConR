...
una variable cuantitativa discreta. Si la variable es el *número de síntomas* a partir de los cuales fueron diagnosticados de esquizofrenia un conjunto de pacientes:
...
set.seed(8818)
df3 <- data.frame(
  normal = rnorm(1000), # simetrica
  exponencial = rexp(n = 1000, rate = .1), # asimetrica a derecha
  gama = rgamma(n = 1000, shape = 20, rate = 2) # asimetrica a izquierda
)

ggplot(df3) + geom_histogram(aes(normal), fill = "green") +
  xlab("x1") + ylab("f") + theme_tufte() + ggtitle("x1, distribución simétrica")

ggplot(df3) + geom_histogram(aes(exponencial), fill = "green") +
  xlab("x2") + ylab("f") + theme_tufte() + ggtitle("x2, distribución asimétrica a la derecha")

ggplot(df3) + geom_histogram(aes(gama), fill = "green") +
...
- x1: media = `r mean(df3$normal)`, mediana = `r median(df3$normal)`
- x2: media = `r mean(df3$exponencial)`, mediana = `r median(df3$exponencial)`
- x3: media = `r mean(df3$gama)`, mediana = `r median(df3$gama)`
...
```{r}
ggplot(df3) + stat_ecdf(aes(normal)) +
  xlab("x1") + ylab("F'") + theme_tufte() + ggtitle("x1, distribución simétrica")

ggplot(df3) + stat_ecdf(aes(exponencial)) +
  xlab("x2") + ylab("F'") + theme_tufte() + ggtitle("x2, distribución asimétrica a la derecha")

ggplot(df3) + stat_ecdf(aes(gama)) +
  xlab("x3") + ylab("F'") + theme_tufte() + ggtitle("x3, distribución asimétrica a la izquierda")
```
...
```{r}
ggplot(df3) + geom_boxplot(aes(1, normal), fill = "green") +
  xlab("x1") + theme_tufte() +
  theme(
...
ggplot(df3) + geom_boxplot(aes(1, exponencial), fill = "green") +
  xlab("x2") + theme_tufte() + theme(
    axis.title.y = element_blank(), axis.text.y = element_blank(),
    axis.ticks.y = element_blank()
  ) + xlab("x2") + ggtitle("x2, distribución asimétrica a la derecha") + coord_flip()

ggplot(df3) + geom_boxplot(aes(1, gama), fill = "green") +
...
Debido a que la letra z se utiliza de manera universal para indicar este o **puntaje z** o **puntuación z**. Esta
...
ggplot(dos.hist) + geom_histogram(aes(unif), fill = "green", bins = 30) + ylab("f") + xlab("") + theme_tufte()
```
